...
**Suspend** (`PATCH …/members/:username { suspended: true }`) ends the membership's access at once and, as in Linear, invalidates the user's credentials everywhere: their sessions, API keys and unused codes are deleted, even if they're still in other workspaces. They stay listed, greyed, so history keeps their name. Reinstating (`suspended: false`) gives back the membership, not the credentials: the person signs in again (from another signed-in device, or the server's CLI). Removing an agent is suspending it; a new token reinstates it.
...
- **One-time codes**: 10 symbols of `A–Z 2–9` without `I O 0 1` (50 bits), shown as `XXXXX-XXXXX`, single-use, 15 minutes. A link is `<origin>/login#<code>`: the fragment never reaches the server or its logs, and the page removes it from the address bar at once.
  ...
  - **Sign-in links** open one person's account: from yourself (to sign in on another device), or from the server's shell (`bun run sign-in-link <username>`, for when nobody can sign in). Suspension deletes your unused codes.
...
| POST | /api/auth/peek | `{ code }` | `CodeInfo` `{ kind, workspace, username, you, needsProfile }` (changes nothing; `you` is who an invite would add when signed in) |
