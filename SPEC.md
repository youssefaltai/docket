...
- **Toolbar**: search (`/` to focus), a "Mine" chip (assignee filter set to you), label and assignee filters, List/Board toggle. People in pickers and filters list you first, marked "(you)".
- **Issue page** (`/issue/BRD-12`): a "Claim" button in the header for a signed-in member on an open, unassigned issue (a 409 shows as a toast naming the holder); inline-editable title; markdown description with edit toggle, saved with `baseUpdatedAt`: on a 409 it refetches, and if only something else changed (e.g. a comment) it saves again on top; if the description itself changed, a banner "Someone changed this description while you were editing." offers "Use theirs" or "Keep mine" (then Save overwrites). Property pickers don't send `baseUpdatedAt`; properties panel (status, priority, assignee, labels, project, parent, blocked by) editable via small popovers; sub-issues; comments thread with composer (`⌘↵` to send).
